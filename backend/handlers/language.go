...
// SupportedLanguage represents a language option
type SupportedLanguage struct {
	Code string `json:"code"`
	Name string `json:"name"`
}

// TranslationEntry is a key-value pair for a translation
...
// LanguageHandler handles i18n API endpoints
...
func (h *LanguageHandler) GetTranslations(c *gin.Context) {
	...
	trans, exists := translations[lang]
	if !exists {
		c.JSON(http.StatusNotFound, gin.H{"error": "Language not supported: " + lang})
		return
	}
...
// GetAllTranslations returns translations for all supported languages
func (h *LanguageHandler) GetAllTranslations(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
